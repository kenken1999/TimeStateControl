...
for i = 1 : 4
    ...
    dk1 = 1;   % 時間刻み
    K1fin = 5;  %シミュレーション終了時間, length(k) = Kfin + 1
    k1 = [0:dk1:K1fin];
    ...
    u1_b1 = ones(length(k1),1) * u1_pm * 0.15; % 並進速度
    u2_b1 = ones(length(k1),1) * u2_pm * (pi/16); % 回転角速度
    ...
    for j = 1:length(k1)
        ...
        % z2_b1(j) = (1/sqrt(3)) * (m_real_2(j) + rho_2(j,2));
