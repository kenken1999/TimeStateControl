...
load('gh_estimation_12pi_change.mat')
...
for z = 1 : 3
    ...
    elseif z == 2
        si = zeros(length(t1),3);
        si(1,:) = [1.15 2 7*pi/12]; %(s1, s2, s3)=(x ,y, θ)の初期値を設定
        k2 = 5;
        k3 = 6;
    else
        si = zeros(length(t1),3);
    ...
    % si(1,:) = [1 2 pi/2]; %(s1, s2, s3)=(x ,y, θ)の初期値を設定
    ...
    for i = 1:length(t1)-1
        ...
        plot(si(i,1),si(i,2), 'o', 'MarkerSize' ,3, 'MarkerFaceColor', 'b','MarkerEdgeColor', 'b');
