...
for t = 1 : iteration
    ...
    if t < iteration
        ...
        % z1,z2,z3の推定結果取得
        for j = 1:length(k)
            z1_b1(j) = l_real(j) + rho(j,1);
            z2_b1(j) = tan(pi/12) * (m_real(j) + rho(j,2));
            z3_b1(j) = n_real(j) + rho(j,3);
        end
...
plot(si_b1(1:length(k)-1,3), g_ans(:), '--m', si_b1(1:length(k)-1,3), g_b1(:),'o','MarkerEdgeColor','red','MarkerFaceColor','red','LineWidth', 1.5)
xlabel("s_3 = \theta [rad]",'fontsize',18)
...
% plot(si_b1(1:length(k)-1,3), cos(si_c1(1:length(k)-1,3)), '--m', si_b1(1:length(k)-1,3), h_b1(:),'o','MarkerEdgeColor','red','MarkerFaceColor','red','LineWidth', 1.5)
% xlabel("s_3 = \theta [rad]", 'fontsize',18)
...
for a = 1:2
    for b = 1:11
        for c = 1:2
            ...
            if b == 1
                plot3(param_s(a,1:11,c,1,1), param_s(a,1:11,c,2,1),param_s(a,1:11,c,3,1),'ko:')
            end
            if a == 1
...
plot3(si_b1(:,1),si_b1(:,2),si_b1(:,3),'-bo','MarkerEdgeColor','blue','MarkerFaceColor','blue','MarkerSize', 4);
...
for a = 1:2
    for b = 1:11
        for c = 1:2
            ...
            if b == 1
                plot3(param_s(a,1:11,c,1,iteration), param_s(a,1:11,c,2,iteration),param_s(a,1:11,c,3,iteration),'ko:')
            end
            if a == 1
