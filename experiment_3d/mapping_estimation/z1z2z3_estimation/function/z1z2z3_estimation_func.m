...
for j = 1 : length(k) - 1
    ...
    H =  transpose(reshape(G,[3,3]));%論文中Aに相当
    y = [si_b1(j,1) - s(1,1,1,1); si_b1(j,2) - s(1,1,1,2); si_b1(j,3) - s(1,1,1,3)];%サンプルと格子点の差
    P = H \ y;%ρに相当

    for b = 1 : 3

        G2 = [s(2,b,1,:) - s(1,b,1,:); s(1,b+1,1,:) - s(1,b,1,:); s(1,b,2,:) - s(1,b,1,:)];
        H2 =  transpose(reshape(G2,[3,3]));%論文中Aに相当
        y2 = [si_b1(j+1,1) - s(1,b,1,1); si_b1(j+1,2) - s(1,b,1,2); si_b1(j+1,3) - s(1,b,1,3)];%サンプルと格子点の差
        P2 = H2 \ y2;%ρに相当
